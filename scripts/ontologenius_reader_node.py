    ...
    def add_scene_node(self, scene_node, agent="myself"):
        ...
        if scene_node.label == "myself":
            ...
            if scene_node.is_located():
                types.append("EnduringThingLocalized")
    ...
    def update_situation(self, situation, agent="myself"):
        ...
        if situation.predicate == "in":
            if not situation.is_finished():
                if situation.subject+"isContainedIn"+situation.object not in self.relations:
                    rospy.loginfo("add: "+situation.subject+" isContainedIn "+situation.object)
                    self.ontologenius_client.feeder.addObjectProperty(situation.subject, "isContainedIn", situation.object)
                    self.relations[situation.subject+"isContainedIn"+situation.object] = True
            else:
                if situation.subject+"isContainedIn"+situation.object in self.relations:
                    rospy.loginfo("remove: "+situation.subject+" isContainedIn "+situation.object)
                    self.ontologenius_client.feeder.removeObjectProperty(situation.subject, "isContainedIn", situation.object)
                    del self.relations[situation.subject+"isContainedIn"+situation.object]
    ...
    def learn_affordance(self, situation, agent="myself"):
        ...
        if situation.predicate == "pick":
            ...
            if situation.object not in self.support:
                rospy.loginfo("add: "+situation.object+" isA "+"PhysicalSupport")
            ...
            if situation.object not in self.container:
                rospy.loginfo("add: "+situation.object+" isA "+"Container")
